...
#[ffi_export]
pub fn cfr_to_bytes_le(cfr: &CFr) -> repr_c::Box<repr_c::Vec<u8>> {
    Box_::new(fr_to_bytes_le(&cfr.0).into())
}

#[ffi_export]
pub fn cfr_to_bytes_be(cfr: &CFr) -> repr_c::Box<repr_c::Vec<u8>> {
    Box_::new(fr_to_bytes_be(&cfr.0).into())
}
...
#[ffi_export]
pub fn vec_cfr_to_bytes_le(vec: &repr_c::Vec<CFr>) -> repr_c::Box<repr_c::Vec<u8>> {
    let vec_fr: Vec<Fr> = vec.iter().map(|cfr| cfr.0).collect();
    Box_::new(crate::utils::vec_fr_to_bytes_le(&vec_fr).into())
}

#[ffi_export]
pub fn vec_cfr_to_bytes_be(vec: &repr_c::Vec<CFr>) -> repr_c::Box<repr_c::Vec<u8>> {
    let vec_fr: Vec<Fr> = vec.iter().map(|cfr| cfr.0).collect();
    Box_::new(crate::utils::vec_fr_to_bytes_be(&vec_fr).into())
}
...
#[ffi_export]
pub fn vec_cfr_free(v: repr_c::Box<repr_c::Vec<CFr>>) {
    drop(v);
}
...
#[ffi_export]
pub fn vec_u8_to_bytes_le(vec: &repr_c::Vec<u8>) -> repr_c::Box<repr_c::Vec<u8>> {
    Box_::new(crate::utils::vec_u8_to_bytes_le(vec).into())
}

#[ffi_export]
pub fn vec_u8_to_bytes_be(vec: &repr_c::Vec<u8>) -> repr_c::Box<repr_c::Vec<u8>> {
    Box_::new(crate::utils::vec_u8_to_bytes_be(vec).into())
}
...
#[ffi_export]
pub fn vec_u8_free(v: repr_c::Box<repr_c::Vec<u8>>) {
    drop(v);
}
...
#[ffi_export]
pub fn ffi_key_gen() -> repr_c::Box<repr_c::Vec<CFr>> {
    let (identity_secret_hash, id_commitment) = keygen();
    Box_::new(vec![CFr(*identity_secret_hash), CFr(id_commitment)].into())
}

#[ffi_export]
pub fn ffi_seeded_key_gen(seed: &repr_c::Vec<u8>) -> repr_c::Box<repr_c::Vec<CFr>> {
    let (identity_secret_hash, id_commitment) = seeded_keygen(seed);
    Box_::new(vec![CFr(identity_secret_hash), CFr(id_commitment)].into())
}

#[ffi_export]
pub fn ffi_extended_key_gen() -> repr_c::Box<repr_c::Vec<CFr>> {
...
#[ffi_export]
pub fn ffi_seeded_extended_key_gen(seed: &repr_c::Vec<u8>) -> repr_c::Box<repr_c::Vec<CFr>> {
    let (identity_trapdoor, identity_nullifier, identity_secret_hash, id_commitment) =
