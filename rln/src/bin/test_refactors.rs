...
fn main() -> Result<(), RLNError> {
    ...
    let rln_stateless = RLNV3::<Stateless, ArkGroth16Backend>::new_stateless(backend.clone());
    ...
    let rln_stateful =
        RLNV3::<Stateful<PmTree>, ArkGroth16Backend>::new_stateful(merkle_tree, backend);
