...
impl RLN<'_> {
    ...
    pub fn verify<R: Read>(&self, mut input_data: R) -> Result<bool> {
        ...
        let proof = ArkProof::deserialize_compressed_unchecked(&mut Cursor::new(&input_byte[..128]))?;
    ...
    pub fn verify_rln_proof<R: Read>(&self, mut input_data: R) -> Result<bool> {
        ...
        let mut all_read = 0;
        let proof =
            ArkProof::deserialize_compressed_unchecked(&mut Cursor::new(&serialized[..128].to_vec()))?;
        all_read += 128;
...
mod test {
    ...
    fn test_groth16_proof() {
        ...
        let proof = ArkProof::deserialize_compressed_unchecked(&mut Cursor::new(&serialized_proof)).unwrap();
