...
mod test {
    ...
    #[test]
    fn test_poseidon2_hash_against_horizenlabs() {
        // Real outputs for input [1n, ..., n] per arity. Arity 2 was generated by running
        // jf-poseidon2 (constants adapted from HorizenLabs) directly; arities 1 and 3 were
        // verified against the sage reference evaluator (the HorizenLabs generation script
        // evaluates poseidon2([0, 1, .., t-1]), exactly these one-shot states).
        let real_horizenlabs_outputs = [
        ...
        for (arity, expected) in real_horizenlabs_outputs
            .iter()
            .enumerate()
            .map(|(i, e)| (i + 1, e))
        {
