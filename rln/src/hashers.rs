...
use zerokit_utils::{
    ...
    poseidon::{Poseidon, Poseidon2, POSEIDON2_ROUND_PARAMS, POSEIDON_ROUND_PARAMS},
};

use crate::circuit::Fr;

/// The Poseidon instance over the Bn254 scalar field, parameterized by [`POSEIDON_ROUND_PARAMS`]
/// (circomlib-compatible round parameters for `1..=16` inputs).
static POSEIDON: LazyLock<Poseidon<Fr>> = LazyLock::new(|| Poseidon::from(&POSEIDON_ROUND_PARAMS));
...
impl ZerokitHasher for PoseidonHash {
    ...
    fn hash(input: &[Fr]) -> Fr {
        POSEIDON
            .hash(input)
            .expect("Input length must be within the supported range 1..=16")
    }
}

/// The Poseidon2 instance over the Bn254 scalar field, parameterized by
/// [`POSEIDON2_ROUND_PARAMS`] (HorizenLabs-compatible round parameters for `1..=3` inputs).
static POSEIDON2: LazyLock<Poseidon2<Fr>> =
    LazyLock::new(|| Poseidon2::from(&POSEIDON2_ROUND_PARAMS));

/// The Poseidon2 hash function over the Bn254 scalar field.
#[derive(Clone, Copy, PartialEq)]
pub struct Poseidon2Hash;

impl ZerokitHasher for Poseidon2Hash {
    type Scalar = Fr;

    fn hash(input: &[Fr]) -> Fr {
        POSEIDON2
            .hash(input)
            .expect("Input length must be within the supported range 1..=3")
    }
}
...
mod test {
    ...
    #[test]
    fn test_poseidon_hash_against_circomlibjs_impl() {
        ...
        //   "
        //
        // Pins circom interop and `SKIP_MATRICES = 0` for every width in `POSEIDON_ROUND_PARAMS`.
        let real_circomlibjs_impl_outputs = [
        ...
        for (arity, expected) in real_circomlibjs_impl_outputs
            .iter()
            .enumerate()
            .map(|(i, e)| (i + 1, e))
        {
    ...
    #[test]
    fn test_poseidon2_hash_against_logos_storage_impl() {
        // Real outputs from rust-poseidon-bn254-pure `hash<n>(1, .., n)` for arity `1..=3`;
        // each value was generated by running the Logos reference implementation directly
        // (https://github.com/logos-storage/rust-poseidon-bn254-pure):
        //
        //   use rust_poseidon_bn254_pure::{bn254::field::Felt, poseidon2::new};
        //   new::hash1(Felt::from_u32(1));
        //   new::hash2(Felt::from_u32(1), Felt::from_u32(2));
        //   new::hash3(Felt::from_u32(1), Felt::from_u32(2), Felt::from_u32(3));
        //
        // Pins the `[inputs.., 0]` compression layout for every width in `POSEIDON2_ROUND_PARAMS`.
        let real_logos_storage_impl_outputs = [
        ...
        for (arity, expected) in real_logos_storage_impl_outputs
            .iter()
            .enumerate()
            .map(|(i, e)| (i + 1, e))
        {
            let input: Vec<Fr> = (1..=arity as u64).map(Fr::from).collect();
            assert_eq!(
                Poseidon2Hash::hash(&input).to_string(),
                *expected,
                "Poseidon2 hash mismatch for arity {arity}"
            );
        }
    }

    #[test]
    fn test_facade_arities_match_concrete_poseidon2() {
        // Every facade method must delegate to `Poseidon2Hash::hash` unchanged.
        let input: Vec<Fr> = (1..=3u64).map(Fr::from).collect();
        assert_eq!(
            Hasher::<Poseidon2Hash>::hash_single(input[0]),
            Poseidon2Hash::hash(&input[..1])
        );
        assert_eq!(
            Hasher::<Poseidon2Hash>::hash_pair(input[0], input[1]),
            Poseidon2Hash::hash(&input[..2])
        );
        for arity in 1..=input.len() {
            assert_eq!(
                Hasher::<Poseidon2Hash>::hash_list(&input[..arity]),
                Poseidon2Hash::hash(&input[..arity]),
                "Facade `hash_list` mismatch for arity {arity}"
            );
        }
    }
