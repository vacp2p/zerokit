...
pub fn pm_tree_benchmark(c: &mut Criterion) {
    let mut tree = PmTree::<PoseidonHash>::default(20).unwrap();

    for i in 0..LEAF_COUNT {
        tree.set(i, LEAVES[i % LEAVES.len()]).unwrap();
    }

    let mut verify_tree = PmTree::<PoseidonHash>::default(20).unwrap();
    for i in 0..LEAF_COUNT {
    ...
    let mut update_next_tree = PmTree::<PoseidonHash>::default(20).unwrap();
