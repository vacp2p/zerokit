...
- **Poseidon2**: Alternative circuits and native hasher using the Poseidon2 permutation
  (about twice as fast for native Merkle hashing); selected via the `_poseidon2` builder
  constructors (`RLNBuilder::stateless_poseidon2()` / `RLNBuilder::stateful_poseidon2()`).
...
- **Pre-compiled Circuits**: Ready-to-use Poseidon and Poseidon2 circuits with Merkle tree
  depth of 10 and 20.
...
The `circom-rln` (<https://github.com/vacp2p/circom-rln>) repository
contains the RLN circuit implementations (`rln_single`, `rln_multi`,
`rln_poseidon2_single`, `rln_poseidon2_multi`) used for the
[pre-compiled](https://github.com/vacp2p/zerokit/tree/master/rln/resources)
RLN circuit resources of zerokit RLN.
...
To customize the circuit parameters, modify the `component main` declaration at the
bottom of the circuit file (`circom-rln/circuits/rln_single.circom`, or the
`rln_multi` / `rln_poseidon2_single` / `rln_poseidon2_multi` variant):
...
git clone https://github.com/vacp2p/circom-rln
...
./scripts/build-circuits.sh <rln_single|rln_multi|rln_poseidon2_single|rln_poseidon2_multi|withdraw>

# Copy the zkey of the built circuit for subsequent steps,
# e.g. zerokit/rln/resources/tree_depth_20/rln_single/rln_final.zkey
cp zkeyFiles/rln_single/final.zkey <path_to_rln_final.zkey>
```
...
As mentioned in step 1, we should use the circuit file of the wanted variant from the
`circom-rln` repository.
...
cargo run -p build-circuit ../circom-rln/circuits/rln_single.circom <path_to_graph.bin>
```
...
This will generate the `rln_final.arkzkey` file next to the input,
which is used by the `rln` module; the intermediate `rln_final.zkey` can be removed.
