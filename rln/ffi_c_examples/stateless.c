...
int main(void)
{
    ...
    Fr *default_leaf = ffi_fr_zero();
    Fr *default_hashes[TREE_DEPTH - 1];
    ...
    Vec_Fr path_elements = ffi_vec_fr_new(TREE_DEPTH);
    ffi_vec_fr_push(&path_elements, default_leaf);
    for (size_t i = 1; i < TREE_DEPTH; i++)
    {
        ffi_vec_fr_push(&path_elements, default_hashes[i - 1]);
    }
    ...
    Fr *computed_root = ffi_poseidon_hash_pair(member.rate_commitment, default_leaf);
    for (size_t i = 1; i < TREE_DEPTH; i++)
    {
        Fr *next_root = ffi_poseidon_hash_pair(computed_root, default_hashes[i - 1]);
        ffi_fr_free(computed_root);
        computed_root = next_root;
    }
    print_fr("computed root", computed_root);
    Vec_Fr roots = ffi_vec_fr_new(1);
    ffi_vec_fr_push(&roots, computed_root);
    ...
    ffi_fr_free(computed_root);
    ffi_vec_fr_free(path_elements);
    for (size_t i = 0; i < TREE_DEPTH - 1; i++)
    {
        ffi_fr_free(default_hashes[i]);
    }
    ffi_fr_free(default_leaf);
    member_free(&member);
