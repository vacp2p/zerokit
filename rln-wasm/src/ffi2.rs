...
impl WasmFr {
    ...
    pub fn from_bigint(value: js_sys::BigInt) -> Result<WasmFr, String> {
        let value_str = value
            .to_string(10)
            .map_err(|e| format!("BigInt conversion error: {:?}", e))?
            .as_string()
            .ok_or("BigInt to string failed")?;

        let bigint = value_str
            .parse::<BigInt>()
            .map_err(|e| format!("Parse error: {:?}", e))?;

        let value_u64 = bigint
            .to_string()
            .parse::<u64>()
            .map_err(|e| format!("U64 conversion error: {:?}", e))?;

        Ok(Self::from_u64(value_u64))
    }
    ...
    pub fn from_bytes_le(bytes: &[u8]) -> Result<WasmFr, String> {
        let inner = Fr::deserialize_compressed(bytes)
    ...
    pub fn from_bytes_be(bytes: &[u8]) -> Result<WasmFr, String> {
        let inner = Fr::deserialize_uncompressed(bytes)
            .map_err(|e| format!("Deserialization error: {:?}", e))?;
        Ok(Self { inner })
    }

    pub fn equals(&self, other: &WasmFr) -> bool {
        self.inner == other.inner
    }

    pub fn add(&self, other: &WasmFr) -> WasmFr {
        Self {
            inner: self.inner + other.inner,
        }
    }

    pub fn sub(&self, other: &WasmFr) -> WasmFr {
        Self {
            inner: self.inner - other.inner,
        }
    }

    pub fn mul(&self, other: &WasmFr) -> WasmFr {
        Self {
            inner: self.inner * other.inner,
        }
    }

    pub fn inverse(&self) -> Result<WasmFr, String> {
        self.inner
            .inverse()
            .ok_or("Element is not invertible".to_string())
...
impl WasmVecFr {
    pub(crate) fn as_slice(&self) -> &[Fr] {
        &self.elements
    }

    pub(crate) fn from_vec(elements: Vec<Fr>) -> Self {
        Self { elements }
    }

    pub(crate) fn into_vec(self) -> Vec<Fr> {
        self.elements
    }
}
