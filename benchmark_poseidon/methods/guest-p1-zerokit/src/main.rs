// Measures zerokit's Poseidon (circomlib-compatible, runtime Grain constant derivation):
// init = constructing the hasher from POSEIDON_ROUND_PARAMS (derives all 16 widths, same
// work as rln's static), steady = `iters` chained hashes of the requested arity.

use ark_bn254::Fr;
use ark_ff::{BigInteger, PrimeField};
use risc0_zkvm::guest::env;
use zerokit_utils::poseidon::{Poseidon, POSEIDON_ROUND_PARAMS};

fn main() {
    let arity: u32 = env::read();
    let iters: u32 = env::read();
    let y = Fr::from(2u64);
    let z = Fr::from(3u64);

    let c0 = env::cycle_count();
    let hasher = Poseidon::<Fr>::from(&POSEIDON_ROUND_PARAMS);
    let c1 = env::cycle_count();

    let mut x = Fr::from(1u64);
    for _ in 0..iters {
        x = match arity {
            1 => hasher.hash(&[x]).unwrap(),
            2 => hasher.hash(&[x, y]).unwrap(),
            _ => hasher.hash(&[x, y, z]).unwrap(),
        };
    }
    let c2 = env::cycle_count();

    let out = x.into_bigint().to_bytes_be();
    env::commit(&(c1 - c0, c2 - c1, out));
}
