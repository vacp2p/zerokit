...
use rln::prelude::{
    default_graph_multi, default_zkey_multi, hash_to_field_le, ArkGroth16Backend, Fr, Hasher,
...
impl RLNSystem {
    fn new() -> Result<Self> {
        ...
        .parse()?;
        let pm_tree =
            PmTree::<SledDB, PoseidonHash>::new(TREE_DEPTH, Fr::default(), pm_tree_config)?;
            ...
            (proof_values.nullifiers(), proof_values.selector_used())
        {
            for (i, (nullifier, active)) in nullifiers.iter().zip(selector.iter()).enumerate() {
                ...
                if let Some(previous_proof_values) = self.used_nullifiers.get(nullifier).cloned() {
                    self.handle_duplicate_nullifier(&previous_proof_values, &proof_values, i)?;
                    return Ok(());
                }
            }
